...
const infoPanel: ComponentDefinition = {
    init: function () {
        ...
        mixins.onMenuButtonClick = mixins.onMenuButtonClick.bind(this);
        mixins.onBackgroundClick = mixins.onBackgroundClick.bind(this);
        this.backgroundEl = document.querySelector("#background");
        for (var i = 0; i < buttonEls.length; ++i) {
            buttonEls[i].addEventListener("click", mixins.onMenuButtonClick);
        }
        this.backgroundEl.addEventListener("click", mixins.onBackgroundClick);
